...
client.on("message", (message) => {
    ...
    if (command === "set-logs") {
        if (!args[0])
            return message.channel.send(`Logs channel for the server is: ${db.get(`logs_${guild.id}`) ? db.get(`logs_${guild.id}`).name : "None"}`);
    ...
    if (command === "questions") {
        ...
        if (args[0] === "add") {
            if (!args[1])
                return message.channel.send("No question was given");
            if (!db.has(`questions_${guild.id}`))
    ...
    if (command === "apps") {
        let func = args[0];
        if (!func)
            return message.channel.send(`Apps is currently: ${db.get(`apps_${guild.id}`) || "off"}`);
    ...
    if (command === "apply") {
        ...
        collector.on("end", (collected) => __awaiter(void 0, void 0, void 0, function* () {
            ...
            collected.forEach((value) => {
                ...
                //   }
                // }
                arr.push(`**Question:** ${questions[count++]}\n**Answer:** ${value.content}`);
            });
            let logs = db.get(`logs_${guild.id}`);
            if (logs) {
                let logsChannel = client.channels.cache.get(logs.id);
